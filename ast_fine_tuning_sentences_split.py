...
ROOT_DIR = "/content/drive/MyDrive/PC-GITA_16kHz/PC-GITA_16kHz/sentences"
...
for root, _, files in os.walk(ROOT_DIR):
    for file in files:
        if file.endswith(".wav"):
            label = infer_label(os.path.join(root, file))
...
class SentencesDataset(Dataset):
    def __init__(self, file_label_pairs, feature_extractor):
    ...
    def __getitem__(self, idx):
        ...
        if sr != 16000:model.save_pretrained("/content/drive/MyDrive/pcgita_ast_sentences")
...
train_dataset = SentencesDataset(train_files, feature_extractor)
val_dataset = SentencesDataset(val_files, feature_extractor)
...
feature_extractor.save_pretrained("/content/drive/MyDrive/pcgita_ast_sentences")
print("Sentences model saved.")
